...
def multi_process(row):
	...
	with Pool(processes = num_processes) as p:
		clean_chunks = p.smap(mad_rfi_sliding, chunks)
...
plt.subplot(2,1,2)
plt.plot(utc_time, ch1_total_power_2, label = f"{window_size} point moving average")
plt.xlabel('Time (UT)', fontsize=12)
plt.ylabel('Power', fontsize=12)
plt.gca().xaxis.set_major_formatter(mdates.DateFormatter('%H:%M'))
plt.xlim(utc_time[0], utc_time[-1])
...
plt.subplot(2,1,2)
plt.plot(utc_time, ch2_total_power_2, label = f"{window_size} point moving average")
plt.xlabel('Time (UT)', fontsize=12)
plt.ylabel('Power', fontsize=12)
plt.gca().xaxis.set_major_formatter(mdates.DateFormatter('%H:%M'))
plt.xlim(utc_time[0], utc_time[-1])
...
plt.subplot(2,1,2)
plt.plot(utc_time, crc_total_power_2, label = f"{window_size} point moving average")
plt.xlabel('Time (UT)', fontsize=12)
plt.ylabel('Power', fontsize=12)
plt.gca().xaxis.set_major_formatter(mdates.DateFormatter('%H:%M'))
plt.xlim(utc_time[0], utc_time[-1])
